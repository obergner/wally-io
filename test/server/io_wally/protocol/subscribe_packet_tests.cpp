...
SCENARIO( "subscribe", "[packets]" )
{
    ...
    {
        const auto remaining_length = uint32_t{20};
        ...
        const auto under_test = subscribe{remaining_length, packet_id, subscriptions};
        ...
        WHEN( "a caller asks for a failure suback" )
        {
            ...
            const auto failure_suback = under_test.fail( );
        ...
        WHEN( "a caller asks for a success suback" )
        {
            ...
            const auto success_suback = under_test.succeed( );
