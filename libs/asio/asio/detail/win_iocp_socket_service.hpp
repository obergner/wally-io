        ...
        class win_iocp_socket_service : public win_iocp_socket_service_base
        {
           public:
            ...
            struct implementation_type : win_iocp_socket_service_base::base_implementation_type
            {
                // Default constructor.
                implementation_type( )
                    : protocol_( endpoint_type( ).protocol( ) ), have_remote_endpoint_( false ), remote_endpoint_( )
                {
                }
            ...
            // Constructor.
            win_iocp_socket_service( asio::io_service& io_service ) : win_iocp_socket_service_base( io_service )
            {
            }
            ...
            endpoint_type remote_endpoint( const implementation_type& impl, asio::error_code& ec ) const
            {
                ...
                std::size_t addr_len = endpoint.capacity( );
                if ( socket_ops::getpeername( impl.socket_, endpoint.data( ), &addr_len, impl.have_remote_endpoint_,
                                              ec ) )
                    return endpoint_type( );
                endpoint.resize( addr_len );
                return endpoint;
                            ...
                            asio::error_code& ec )
            {
                ...
                return socket_ops::sync_sendto( impl.socket_, impl.state_, bufs.buffers( ), bufs.count( ), flags,
                                ...
                                Handler& handler )
            {
                ...
                ASIO_HANDLER_CREATION( ( p.p, "socket", &impl, "async_send_to(null_buffers)" ) );
                                 ...
                                 asio::error_code& ec )
            {
                ...
                std::size_t bytes_recvd =
                    socket_ops::sync_recvfrom( impl.socket_, impl.state_, bufs.buffers( ), bufs.count( ), flags,
            ...
            {
                // Allocate and construct an operation to wrap the handler.
                typedef win_iocp_socket_recvfrom_op<MutableBufferSequence, endpoint_type, Handler> op;
                typename op::ptr p = {asio::detail::addressof( handler ),
                                      asio_handler_alloc_helpers::allocate( sizeof( op ), handler ), 0};
                                     ...
                                     Handler& handler )
            {
                ...
                ASIO_HANDLER_CREATION( ( p.p, "socket", &impl, "async_receive_from(null_buffers)" ) );
            ...
            void async_accept( implementation_type& impl, Socket& peer, endpoint_type* peer_endpoint, Handler& handler )
            {
                ...
                p.p = new ( p.v )
                    op( *this, impl.socket_, peer, impl.protocol_, peer_endpoint, enable_connection_aborted, handler );
