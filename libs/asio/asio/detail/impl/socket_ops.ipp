    ...
    namespace detail
    {
        namespace socket_ops
        {
                                       ...
                                       asio::error_code& ec )
            {
                ...
                if ( !ec )
                {
                    ...
                    if ( addr && addrlen )
                    {
                        ...
                        GetAcceptExSockaddrs( output_buffer, 0, address_length, address_length, &local_addr,
                    ...
                    socket_ops::setsockopt( new_socket, state, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, &update_ctx_param,
                                            sizeof( SOCKET ), ec );
                }
            }
            ...
            int bind( socket_type s, const socket_addr_type* addr, std::size_t addrlen, asio::error_code& ec )
            {
                ...
                clear_last_error( );
                int result = error_wrapper( call_bind( &msghdr::msg_namelen, s, addr, addrlen ), ec );
                if ( result == 0 )
                    ec = asio::error_code( );
                return result;
            }
            ...
            int connect( socket_type s, const socket_addr_type* addr, std::size_t addrlen, asio::error_code& ec )
            {
                ...
                clear_last_error( );
                int result = error_wrapper( call_connect( &msghdr::msg_namelen, s, addr, addrlen ), ec );
                if ( result == 0 )
                    ec = asio::error_code( );
            ...
            void sync_connect( socket_type s, const socket_addr_type* addr, std::size_t addrlen, asio::error_code& ec )
            {
                ...
                if ( socket_ops::getsockopt( s, 0, SOL_SOCKET, SO_ERROR, &connect_error, &connect_error_len, ec ) ==
                     socket_error_retval )
                    return;
            ...
            void complete_iocp_connect( socket_type s, asio::error_code& ec )
            {
                ...
                if ( !ec )
                {
                    ...
                    socket_ops::setsockopt( s, state, SOL_SOCKET, so_update_connect_context, 0, 0, ec );
                }
            }
        ...
        // || defined(__SYMBIAN32__)
            ...
            size_t sync_recv( socket_type s,
                              state_type state,
                              buf* bufs,
                              size_t count,
                              int flags,
                                     ...
                                     size_t bytes_transferred )
            {
                ...
                else if ( !ec && bytes_transferred == 0 && ( state & stream_oriented ) != 0 && !all_empty )
                {
                    ec = asio::error::eof;
                }
            }
            ...
            signed_size_type recvfrom( socket_type s,
                                       buf* bufs,
                                       size_t count,
                                       int flags,
                                       socket_addr_type* addr,
                                       ...
                                       asio::error_code& ec )
            {
                ...
                int result = error_wrapper(
                    ::WSARecvFrom( s, bufs, recv_buf_count, &bytes_transferred, &recv_flags, addr, &tmp_addrlen, 0, 0 ),
                    ec );
                *addrlen = (std::size_t)tmp_addrlen;
                if ( ec.value( ) == ERROR_NETNAME_DELETED )
                    ec = asio::error::connection_reset;
            ...
            size_t sync_recvfrom( socket_type s,
                                  state_type state,
                                  buf* bufs,
                                  size_t count,
                                  int flags,
                                  ...
                                  asio::error_code& ec )
            {
                ...
                {
                    // Try to complete the operation without blocking.
                    signed_size_type bytes = socket_ops::recvfrom( s, bufs, count, flags, addr, addrlen, ec );
            ...
            void complete_iocp_recvfrom( const weak_cancel_token_type& cancel_token, asio::error_code& ec )
            {
                // Map non-portable errors to their portable counterparts.
                if ( ec.value( ) == ERROR_NETNAME_DELETED )
                {
            ...
            signed_size_type recvmsg( socket_type s,
                                      buf* bufs,
                                      size_t count,
                                      int in_flags,
                                      int& out_flags,
            ...
            size_t sync_recvmsg( socket_type s,
                                 state_type state,
                                 buf* bufs,
                                 size_t count,
                                 int in_flags,
                                 ...
                                 asio::error_code& ec )
            {
                ...
                {
                    // Try to complete the operation without blocking.
                    signed_size_type bytes = socket_ops::recvmsg( s, bufs, count, in_flags, out_flags, ec );
            ...
            void complete_iocp_recvmsg( const weak_cancel_token_type& cancel_token, asio::error_code& ec )
            {
                // Map non-portable errors to their portable counterparts.
                if ( ec.value( ) == ERROR_NETNAME_DELETED )
                {
            ...
            signed_size_type send( socket_type s, const buf* bufs, size_t count, int flags, asio::error_code& ec )
            {
                ...
                int result = error_wrapper(
                    ::WSASend( s, const_cast<buf*>( bufs ), send_buf_count, &bytes_transferred, send_flags, 0, 0 ),
                    ec );
                if ( ec.value( ) == ERROR_NETNAME_DELETED )
                    ec = asio::error::connection_reset;
                else if ( ec.value( ) == ERROR_PORT_UNREACHABLE )
            ...
            size_t sync_send( socket_type s,
                              state_type state,
                              const buf* bufs,
                              size_t count,
                              int flags,
            ...
            void complete_iocp_send( const weak_cancel_token_type& cancel_token, asio::error_code& ec )
            {
                // Map non-portable errors to their portable counterparts.
                if ( ec.value( ) == ERROR_NETNAME_DELETED )
                {
            ...
            signed_size_type sendto( socket_type s,
                                     const buf* bufs,
                                     size_t count,
                                     int flags,
                                     const socket_addr_type* addr,
            ...
            size_t sync_sendto( socket_type s,
                                state_type state,
                                const buf* bufs,
                                size_t count,
                                int flags,
            ...
            socket_type socket( int af, int type, int protocol, asio::error_code& ec )
            {
                ...
                if ( af == ASIO_OS_DEF( AF_INET6 ) )
                {
                    ...
                    DWORD optval = 0;
                    ::setsockopt( s, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<const char*>( &optval ),
                                  sizeof( optval ) );
                }
            ...
            inline int call_setsockopt( SockLenType msghdr::*,
                                        ...
                                        std::size_t optlen )
            {
                return ::setsockopt( s, level, optname, (const char*)optval, (SockLenType)optlen );
            }

            int setsockopt( socket_type s,
                            state_type& state,
                            int level,
                            int optname,
                            const void* optval,
                            ...
                            asio::error_code& ec )
            {
                ...
                if ( HMODULE winsock_module = ::GetModuleHandleA( "ws2_32" ) )
                {
                    ...
                    {
                        clear_last_error( );
                        return error_wrapper( sso( s, level, optname, reinterpret_cast<const char*>( optval ),
                ...
                clear_last_error( );
                int result =
                    error_wrapper( call_setsockopt( &msghdr::msg_namelen, s, level, optname, optval, optlen ), ec );
                if ( result == 0 )
                {
                    ec = asio::error_code( );

#if defined( __MACH__ ) && defined( __APPLE__ ) || defined( __NetBSD__ ) || defined( __FreeBSD__ ) || \
    defined( __OpenBSD__ )
                    // To implement portable behaviour for SO_REUSEADDR with UDP sockets we
                    // need to also set SO_REUSEPORT on BSD-based platforms.
                    if ( ( state & datagram_oriented ) && level == SOL_SOCKET && optname == SO_REUSEADDR )
                    {
                        call_setsockopt( &msghdr::msg_namelen, s, SOL_SOCKET, SO_REUSEPORT, optval, optlen );
                    }
#endif
                }
                            ...
                            asio::error_code& ec )
            {
                ...
                if ( HMODULE winsock_module = ::GetModuleHandleA( "ws2_32" ) )
                {
                    ...
                    if ( gso_t gso = ( gso_t )::GetProcAddress( winsock_module, "getsockopt" ) )
                    {
                        ...
                        int result = error_wrapper(
                            gso( s, level, optname, reinterpret_cast<char*>( optval ), &tmp_optlen ), ec );
                ...
                clear_last_error( );
                int result =
                    error_wrapper( call_getsockopt( &msghdr::msg_namelen, s, level, optname, optval, optlen ), ec );
                             ...
                             asio::error_code& ec )
            {
                ...
                if ( cached )
                {
                    ...
                    if ( socket_ops::getsockopt( s, 0, SOL_SOCKET, SO_CONNECT_TIME, &connect_time, &connect_time_len,
                                                 ec ) == socket_error_retval )
                    {
                        return socket_error_retval;
                    }
        ...
        // || defined(__CYGWIN__)
                ...
                clear_last_error( );
                int result = error_wrapper( call_getpeername( &msghdr::msg_namelen, s, addr, addrlen ), ec );
                if ( result == 0 )
                    ec = asio::error_code( );
                return result;
            }
            ...
            int getsockname( socket_type s, socket_addr_type* addr, std::size_t* addrlen, asio::error_code& ec )
            {
                ...
                clear_last_error( );
                int result = error_wrapper( call_getsockname( &msghdr::msg_namelen, s, addr, addrlen ), ec );
                if ( result == 0 )
                    ec = asio::error_code( );
                return result;
            }
    ...
    defined( __OpenBSD__ )
        ...
        // || defined(__SYMBIAN32__)
                                   ...
                                   asio::error_code& ec )
            {
                ...
                {
                    sprintf_s( dest, length, "%u.%u.%u.%u", bytes[0], bytes[1], bytes[2], bytes[3] );
                    return dest;
                }
            ...
            inline hostent* gethostbyaddr( const char* addr,
                                           int length,
                                           int af,
                                           hostent* result,
                                           char* buffer,
            ...
            inline int gai_nsearch( const char* host, const addrinfo_type* hints, gai_search ( &search )[2] )
            {
                int search_count = 0;
                if ( host == 0 || host[0] == '\0' )
                {
            ...
            inline int gai_aistruct( addrinfo_type*** next, const addrinfo_type* hints, const void* addr, int family )
            {
                using namespace std;
            ...
            inline int gai_port( addrinfo_type* aihead, int port, int socktype )
            {
                ...
                for ( addrinfo_type* ai = aihead; ai; ai = ai->ai_next )
                {
                    ...
                    switch ( ai->ai_family )
                    {
                        case ASIO_OS_DEF( AF_INET ):
                        {
                            sockaddr_in4_type* sinptr = reinterpret_cast<sockaddr_in4_type*>( ai->ai_addr );
                        ...
                        case ASIO_OS_DEF( AF_INET6 ):
                        {
                            sockaddr_in6_type* sin6ptr = reinterpret_cast<sockaddr_in6_type*>( ai->ai_addr );
            ...
            inline int gai_serv( addrinfo_type* aihead, const addrinfo_type* hints, const char* serv )
            {
                using namespace std;
            ...
            inline int gai_echeck( const char* host,
                                   const char* service,
                                   int flags,
                                   int family,
                                   int socktype,
            ...
            inline int getaddrinfo_emulation( const char* host,
                                              const char* service,
                                              ...
                                              addrinfo_type** result )
            {
                ...
                int rc =
                    gai_echeck( host, service, hints.ai_flags, hints.ai_family, hints.ai_socktype, hints.ai_protocol );
                if ( rc != 0 )
                {
                ...
                for ( gai_search* sptr = search; sptr < search + search_count; ++sptr )
                {
                    ...
                    asio::error_code ec;
                    if ( socket_ops::inet_pton( ASIO_OS_DEF( AF_INET ), sptr->host, &inaddr, 0, ec ) == 1 )
                    {
                        if ( hints.ai_family != ASIO_OS_DEF( AF_UNSPEC ) && hints.ai_family != ASIO_OS_DEF( AF_INET ) )
                        {
                            freeaddrinfo_emulation( aihead );
                            gai_free( canon );
                            return EAI_FAMILY;
                    ...
                    if ( socket_ops::inet_pton( ASIO_OS_DEF( AF_INET6 ), sptr->host, &in6addr, 0, ec ) == 1 )
                    {
                        if ( hints.ai_family != ASIO_OS_DEF( AF_UNSPEC ) && hints.ai_family != ASIO_OS_DEF( AF_INET6 ) )
                        {
                            freeaddrinfo_emulation( aihead );
                            gai_free( canon );
                            return EAI_FAMILY;
                    ...
                    hostent hent;
                    char hbuf[8192] = "";
                    hostent* hptr = socket_ops::gethostbyname( sptr->host, sptr->family, &hent, hbuf, sizeof( hbuf ),
                                                               hints.ai_flags, ec );
                    if ( hptr == 0 )
                    {
                        if ( search_count == 2 )
                    ...
                    if ( hints.ai_family != ASIO_OS_DEF( AF_UNSPEC ) && hints.ai_family != hptr->h_addrtype )
                    {
                        freeaddrinfo_emulation( aihead );
                        gai_free( canon );
                                                           ...
                                                           asio::error_code& ec )
            {
                ...
                if ( host && hostlen > 0 )
                {
                    ...
                    else
                    {
                        hostent hent;
                        char hbuf[8192] = "";
                        hostent* hptr = socket_ops::gethostbyaddr( addr, static_cast<int>( addr_len ), sa->sa_family,
                                          ...
                                          asio::error_code& ec )
            {
                ...
                typedef int( WSAAPI * gai_t )( const char*, const char*, const addrinfo_type*, addrinfo_type** );
                if ( HMODULE winsock_module = ::GetModuleHandleA( "ws2_32" ) )
                {
            ...
            asio::error_code background_getaddrinfo( const weak_cancel_token_type& cancel_token,
                                                     const char* host,
                                                     const char* service,
                                                     const addrinfo_type& hints,
                                                     addrinfo_type** result,
                                          ...
                                          asio::error_code& ec )
            {
                ...
                // Building for Windows XP, Windows Server 2003, or later.
                clear_last_error( );
                int error = ::getnameinfo( addr, static_cast<socklen_t>( addrlen ), host, static_cast<DWORD>( hostlen ),
                                           serv, static_cast<DWORD>( servlen ), flags );
                return ec = translate_addrinfo_error( error );
#else
                // Building for Windows 2000 or earlier.
                typedef int( WSAAPI * gni_t )( const socket_addr_type*, int, char*, DWORD, char*, DWORD, int );
                if ( HMODULE winsock_module = ::GetModuleHandleA( "ws2_32" ) )
                {
                    if ( gni_t gni = ( gni_t )::GetProcAddress( winsock_module, "getnameinfo" ) )
                    {
                        clear_last_error( );
                        int error = gni( addr, static_cast<int>( addrlen ), host, static_cast<DWORD>( hostlen ), serv,
                                         static_cast<DWORD>( servlen ), flags );
                        return ec = translate_addrinfo_error( error );
                    }
                }
                clear_last_error( );
                return getnameinfo_emulation( addr, addrlen, host, hostlen, serv, servlen, flags, ec );
#endif
#elif !defined( ASIO_HAS_GETADDRINFO )
                using namespace std;  // For memcpy.
            ...
            asio::error_code sync_getnameinfo( const socket_addr_type* addr,
                                               std::size_t addrlen,
                                               char* host,
                                               std::size_t hostlen,
                                               char* serv,
                                               ...
                                               asio::error_code& ec )
            {
                ...
                socket_ops::getnameinfo( addr, addrlen, host, hostlen, serv, servlen, flags, ec );
                if ( ec )
                {
                    socket_ops::getnameinfo( addr, addrlen, host, hostlen, serv, servlen, flags | NI_NUMERICSERV, ec );
                }

                return ec;
            }

            asio::error_code background_getnameinfo( const weak_cancel_token_type& cancel_token,
                                                     const socket_addr_type* addr,
                                                     std::size_t addrlen,
                                                     char* host,
                                                     std::size_t hostlen,
                                                     ...
                                                     asio::error_code& ec )
            {
                ...
                else
                {
                    ...
                    if ( ec )
                    {
                        socket_ops::getnameinfo( addr, addrlen, host, hostlen, serv, servlen, flags | NI_NUMERICSERV,
                                                 ec );
                    }
                }
