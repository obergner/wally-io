...
namespace asio
{
    namespace detail
    {
        ...
        void dev_poll_reactor::start_op( int op_type,
                                         socket_type descriptor,
                                         dev_poll_reactor::per_descriptor_data&,
                                         reactor_op* op,
        ...
        void dev_poll_reactor::cancel_ops( socket_type descriptor, dev_poll_reactor::per_descriptor_data& )
        {
            asio::detail::mutex::scoped_lock lock( mutex_ );
        ...
        void dev_poll_reactor::deregister_internal_descriptor( socket_type descriptor,
                                                               dev_poll_reactor::per_descriptor_data& )
        {
            asio::detail::mutex::scoped_lock lock( mutex_ );
        ...
        void dev_poll_reactor::run( bool block, op_queue<operation>& ops )
        {
            ...
            if ( events_size > 0 )
            {
                ...
                {
                    asio::error_code ec = asio::error_code( errno, asio::error::get_system_category( ) );
            ...
            for ( int i = 0; i < num_events; ++i )
            {
                ...
                else
                {
                    ...
                    if ( ( events[i].events & ( POLLERR | POLLHUP ) ) != 0 && !more_except && !more_reads &&
                         !more_writes )
                    {
        ...
        void dev_poll_reactor::cancel_ops_unlocked( socket_type descriptor, const asio::error_code& ec )
        {
        ...
        {
            hash_map<int, std::size_t>::iterator iter = pending_event_change_index_.find( descriptor );
