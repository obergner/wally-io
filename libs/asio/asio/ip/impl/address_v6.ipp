...
namespace asio
{
    namespace ip
    {
        ...
        address_v6::address_v6( const address_v6::bytes_type& bytes, unsigned long scope ) : scope_id_( scope )
        {
#if UCHAR_MAX > 0xFF
        ...
        address_v6::address_v6( const address_v6& other ) : addr_( other.addr_ ), scope_id_( other.scope_id_ )
        {
        }

#if defined( ASIO_HAS_MOVE )
        address_v6::address_v6( address_v6&& other ) : addr_( other.addr_ ), scope_id_( other.scope_id_ )
        {
        }
        ...
        std::string address_v6::to_string( asio::error_code& ec ) const
        {
            ...
            const char* addr = asio::detail::socket_ops::inet_ntop( ASIO_OS_DEF( AF_INET6 ), &addr_, addr_str,
        ...
        address_v6 address_v6::from_string( const char* str, asio::error_code& ec )
        {
            address_v6 tmp;
            if ( asio::detail::socket_ops::inet_pton( ASIO_OS_DEF( AF_INET6 ), str, &tmp.addr_, &tmp.scope_id_, ec ) <=
                 0 )
                return address_v6( );
            return tmp;
        }
        ...
        address_v6 address_v6::from_string( const std::string& str, asio::error_code& ec )
        {
        ...
        bool operator==( const address_v6& a1, const address_v6& a2 )
        {
            ...
            return memcmp( &a1.addr_, &a2.addr_, sizeof( asio::detail::in6_addr_type ) ) == 0 &&
        ...
        {
            using namespace std;  // For memcmp.
            int memcmp_result = memcmp( &a1.addr_, &a2.addr_, sizeof( asio::detail::in6_addr_type ) );
            if ( memcmp_result < 0 )
                return true;
            if ( memcmp_result > 0 )
                return false;
