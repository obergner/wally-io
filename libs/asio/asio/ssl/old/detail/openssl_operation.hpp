            ...
            namespace detail
            {
                ...
                class net_buffer
                {
                   ...
                   public:
                    ...
                    {
                        return ( NET_BUF_SIZE - ( data_end_ - buf_ ) );
                    }
                    size_t get_data_len( )
                    {
                    ...
                    {
                        data_end_ += count;
                        data_end_ = data_end_ > ( buf_ + NET_BUF_SIZE ) ? ( buf_ + NET_BUF_SIZE ) : data_end_;
                    }
                ...
                class openssl_operation
                {
                   public:
                    ...
                    {
                        write_ =
                            boost::bind( &openssl_operation::do_async_write, this, boost::arg<1>( ), boost::arg<2>( ) );
                        read_ = boost::bind( &openssl_operation::do_async_read, this );
                        handler_ = boost::bind( &openssl_operation::async_user_handler, this, boost::arg<1>( ),
                                                boost::arg<2>( ) );
                    }
                    ...
                    {
                        write_ =
                            boost::bind( &openssl_operation::do_sync_write, this, boost::arg<1>( ), boost::arg<2>( ) );
                        read_ = boost::bind( &openssl_operation::do_sync_read, this );
                        handler_ = boost::bind( &openssl_operation::sync_user_handler, this, boost::arg<1>( ),
                                                boost::arg<2>( ) );
                    }
                    ...
                    int start( )
                    {
                        ...
                        int error_code = !is_operation_done ? ::SSL_get_error( session_, rc ) : 0;
                        int sys_error_code = ERR_get_error( );

                        if ( error_code == SSL_ERROR_SSL )
                            return handler_( asio::error_code( sys_error_code, asio::error::get_ssl_category( ) ), rc );
                        ...
                        if ( !is_operation_done && !is_read_needed && !is_write_needed && !is_shut_down_sent )
                        {
                            ...
                            {
                                return handler_( asio::error_code( sys_error_code, asio::error::system_category ), rc );
                            }
                            else
                            {
                                return handler_( asio::error_code( sys_error_code, asio::error::get_ssl_category( ) ),
                                                 rc );
                            }
                        }
                        ...
                        if ( !is_operation_done && !is_write_needed )
                        {
                            ...
                            if ( recv_buf_.get_data_len( ) > 0 )
                            {
                                ...
                                int written =
                                    ::BIO_write( ssl_bio_, recv_buf_.get_data_start( ), recv_buf_.get_data_len( ) );
                   ...
                   private:
                    ...
                    int do_async_write( bool is_operation_done, int rc )
                    {
                        ...
                        if ( len )
                        {
                            ...
                            len = (int)send_buf_.get_unused_len( ) > len ? len : send_buf_.get_unused_len( );
                            ...
                            if ( len > 0 )
                            {
                                ...
                                ASIO_ASSERT( strand_ );
                                asio::async_write(
                                    socket_, asio::buffer( data_start, len ),
                                    strand_->wrap( boost::bind( &openssl_operation::async_write_handler, this,
                                                                is_operation_done, rc, asio::placeholders::error,
                    ...
                    void async_write_handler( bool is_operation_done,
                                              int rc,
                    ...
                    int do_async_read( )
                    {
                        ...
                        socket_.async_read_some(
                            ...
                            strand_->wrap( boost::bind( &openssl_operation::async_read_handler, this,
                    ...
                    int do_sync_write( bool is_operation_done, int rc )
                    {
                        ...
                        if ( len )
                        {
                            ...
                            {
                                size_t sent_len =
                                    asio::write( socket_, asio::buffer( send_buf_.get_unused_start( ), len ) );
                    ...
                    int do_sync_read( )
                    {
                        ...
                        // Pass the received data to SSL
                        int written = ::BIO_write( ssl_bio_, recv_buf_.get_data_start( ), recv_buf_.get_data_len( ) );
