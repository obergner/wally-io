    ...
    namespace detail
    {
        ...
        class resolver_service : public resolver_service_base
        {
           public:
            ...
            // Constructor.
            resolver_service( asio::io_service& io_service ) : resolver_service_base( io_service )
            {
            }
            ...
            iterator_type resolve( implementation_type&, const query_type& query, asio::error_code& ec )
            {
                ...
                socket_ops::getaddrinfo( query.host_name( ).c_str( ), query.service_name( ).c_str( ), query.hints( ),
                                         &address_info, ec );
                auto_addrinfo auto_address_info( address_info );

                return ec ? iterator_type( )
                          : iterator_type::create( address_info, query.host_name( ), query.service_name( ) );
            }
            ...
            iterator_type resolve( implementation_type&, const endpoint_type& endpoint, asio::error_code& ec )
            {
                ...
                socket_ops::sync_getnameinfo( endpoint.data( ), endpoint.size( ), host_name, NI_MAXHOST, service_name,
                                              NI_MAXSERV, endpoint.protocol( ).type( ), ec );

                return ec ? iterator_type( ) : iterator_type::create( endpoint, host_name, service_name );
            }
