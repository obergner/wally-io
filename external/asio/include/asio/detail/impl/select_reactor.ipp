...
namespace asio
{
    namespace detail
    {
              ...
              shutdown_( false )
        {
            ...
            thread_ = new asio::detail::thread( bind_handler( &select_reactor::call_run_thread, this ) );
#endif  // defined(ASIO_HAS_IOCP)
        }
        ...
        int select_reactor::register_descriptor( socket_type, select_reactor::per_descriptor_data& )
        {
            return 0;
        }
        ...
        void select_reactor::cancel_ops( socket_type descriptor, select_reactor::per_descriptor_data& )
        {
            asio::detail::mutex::scoped_lock lock( mutex_ );
            cancel_ops_unlocked( descriptor, asio::error::operation_aborted );
        }

        void select_reactor::deregister_descriptor( socket_type descriptor, select_reactor::per_descriptor_data&, bool )
        {
            asio::detail::mutex::scoped_lock lock( mutex_ );
            cancel_ops_unlocked( descriptor, asio::error::operation_aborted );
        }

        void select_reactor::deregister_internal_descriptor( socket_type descriptor,
                                                             select_reactor::per_descriptor_data& )
        {
            asio::detail::mutex::scoped_lock lock( mutex_ );
            op_queue<operation> ops;
        ...
        void select_reactor::run( bool block, op_queue<operation>& ops )
        {
            ...
            asio::error_code ec;
            int retval = socket_ops::select( static_cast<int>( max_fd + 1 ), fd_sets_[read_op], fd_sets_[write_op],
                                             fd_sets_[except_op], tv, ec );
        ...
        void select_reactor::cancel_ops_unlocked( socket_type descriptor, const asio::error_code& ec )
        {
            bool need_interrupt = false;
            op_queue<operation> ops;
            for ( int i = 0; i < max_ops; ++i )
                need_interrupt = op_queue_[i].cancel_operations( descriptor, ops, ec ) || need_interrupt;
            io_service_.post_deferred_completions( ops );
            if ( need_interrupt )
                interrupter_.interrupt( );
        }
