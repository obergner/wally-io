...
namespace asio
{
    namespace detail
    {
        ...
        win_iocp_socket_service_base::win_iocp_socket_service_base( asio::io_service& io_service )
        ...
        void win_iocp_socket_service_base::construct( win_iocp_socket_service_base::base_implementation_type& impl )
        {
        ...
        void win_iocp_socket_service_base::destroy( win_iocp_socket_service_base::base_implementation_type& impl )
        {
            close_for_destruction( impl );
            ...
            asio::error_code& ec )
        {
            if ( is_open( impl ) )
            {
                ...
                reactor* r = static_cast<reactor*>(
                    interlocked_compare_exchange_pointer( reinterpret_cast<void**>( &reactor_ ), 0, 0 ) );
                if ( r )
        ...
        void win_iocp_socket_service_base::start_send_op( win_iocp_socket_service_base::base_implementation_type& impl,
                                                          WSABUF* buffers,
                                                          ...
                                                          operation* op )
        {
            ...
            else
            {
                ...
                int result = ::WSASend( impl.socket_, buffers, static_cast<DWORD>( buffer_count ), &bytes_transferred,
                                        flags, op, 0 );
            ...
            operation* op )
        {
            ...
            {
                DWORD bytes_transferred = 0;
                int result = ::WSASendTo( impl.socket_, buffers, static_cast<DWORD>( buffer_count ), &bytes_transferred,
                                          flags, addr, addrlen, op, 0 );
                DWORD last_error = ::WSAGetLastError( );
                if ( last_error == ERROR_PORT_UNREACHABLE )
                    last_error = WSAECONNREFUSED;
                if ( result != 0 && last_error != WSA_IO_PENDING )
            ...
            operation* op )
        {
            ...
            else
            {
                ...
                int result = ::WSARecv( impl.socket_, buffers, static_cast<DWORD>( buffer_count ), &bytes_transferred,
                                        &recv_flags, op, 0 );
                DWORD last_error = ::WSAGetLastError( );
            ...
            reactor_op* op )
        {
            ...
            {
                start_reactor_op(
                    impl, ( flags & socket_base::message_out_of_band ) ? reactor::except_op : reactor::read_op, op );
            }
        }
            ...
            operation* op )
        {
            ...
            else
            {
                DWORD bytes_transferred = 0;
                DWORD recv_flags = flags;
                int result = ::WSARecvFrom( impl.socket_, buffers, static_cast<DWORD>( buffer_count ),
            ...
            operation* op )
        {
            ...
            else
            {
                ...
                {
                    DWORD bytes_read = 0;
                    BOOL result = ::AcceptEx( impl.socket_, new_socket.get( ), output_buffer, 0, address_length,
                                              address_length, &bytes_read, op );
                    DWORD last_error = ::WSAGetLastError( );
        ...
        void win_iocp_socket_service_base::restart_accept_op( socket_type s,
                                                              socket_holder& new_socket,
                                                              int family,
                                                              int type,
                                                              int protocol,
                                                              ...
                                                              operation* op )
        {
            ...
            {
                DWORD bytes_read = 0;
                BOOL result = ::AcceptEx( s, new_socket.get( ), output_buffer, 0, address_length, address_length,
                                          &bytes_read, op );
                DWORD last_error = ::WSAGetLastError( );
                if ( !result && last_error != WSA_IO_PENDING )
                    iocp_service_.on_completion( op, last_error );
            ...
            win_iocp_socket_connect_op_base* op )
        {
            ...
            if ( family == ASIO_OS_DEF( AF_INET ) || family == ASIO_OS_DEF( AF_INET6 ) )
            {
                if ( connect_ex_fn connect_ex = get_connect_ex( impl, type ) )
                {
                    ...
                    socket_ops::bind( impl.socket_, &a.base,
                                      family == ASIO_OS_DEF( AF_INET ) ? sizeof( a.v4 ) : sizeof( a.v6 ), op->ec_ );
                    ...
                    BOOL result = connect_ex( impl.socket_, addr, static_cast<int>( addrlen ), 0, 0, 0, op );
                    DWORD last_error = ::WSAGetLastError( );
                    if ( !result && last_error != WSA_IO_PENDING )
                        iocp_service_.on_completion( op, last_error );
                    else
                 ...
                 socket_ops::set_internal_non_blocking( impl.socket_, impl.state_, true, op->ec_ ) )
            {
                ...
                {
                    if ( op->ec_ == asio::error::in_progress || op->ec_ == asio::error::would_block )
                    {
                        op->ec_ = asio::error_code( );
                        r.start_op( reactor::connect_op, impl.socket_, impl.reactor_data_, op, false, false );
                        return;
                    }
                }
            }
        ...
        win_iocp_socket_service_base::connect_ex_fn win_iocp_socket_service_base::get_connect_ex(
            win_iocp_socket_service_base::base_implementation_type& impl,
            int type )
        {
        ...
        void* win_iocp_socket_service_base::interlocked_compare_exchange_pointer( void** dest, void* exch, void* cmp )
        {
#if defined( _M_IX86 )
            return reinterpret_cast<void*>( InterlockedCompareExchange(
                reinterpret_cast<PLONG>( dest ), reinterpret_cast<LONG>( exch ), reinterpret_cast<LONG>( cmp ) ) );
#else
            return InterlockedCompareExchangePointer( dest, exch, cmp );
#endif
        }

        void* win_iocp_socket_service_base::interlocked_exchange_pointer( void** dest, void* val )
        {
#if defined( _M_IX86 )
            return reinterpret_cast<void*>(
                InterlockedExchange( reinterpret_cast<PLONG>( dest ), reinterpret_cast<LONG>( val ) ) );
#else
