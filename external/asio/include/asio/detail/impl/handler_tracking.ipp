    ...
    namespace detail
    {
        ...
        struct handler_tracking_timestamp
        {
            ...
            handler_tracking_timestamp( )
            {
                ...
                boost::posix_time::time_duration now = boost::posix_time::microsec_clock::universal_time( ) - epoch;
#elif defined( ASIO_HAS_STD_CHRONO )
                typedef chrono_time_traits<std::chrono::system_clock, asio::wait_traits<std::chrono::system_clock>>
                    traits_helper;
                traits_helper::posix_time_duration now( std::chrono::system_clock::now( ).time_since_epoch( ) );
#elif defined( ASIO_HAS_BOOST_CHRONO )
                typedef chrono_time_traits<boost::chrono::system_clock, asio::wait_traits<boost::chrono::system_clock>>
                    traits_helper;
                traits_helper::posix_time_duration now( boost::chrono::system_clock::now( ).time_since_epoch( ) );
#endif
...
#endif  // defined(ASIO_WINDOWS)
                timestamp.seconds, timestamp.microseconds, current_id, h->id_, object_type, object, op_name );
        }
...
#endif  // defined(ASIO_WINDOWS)
                timestamp.seconds, timestamp.microseconds, id_, ec.category( ).name( ), ec.value( ) );
...
#endif  // defined(ASIO_WINDOWS)
                timestamp.seconds, timestamp.microseconds, id_, ec.category( ).name( ), ec.value( ), signal_number );
...
#endif  // defined(ASIO_WINDOWS)
                timestamp.seconds, timestamp.microseconds, id_, ec.category( ).name( ), ec.value( ), arg );
        ...
        void handler_tracking::operation( const char* object_type, void* object, const char* op_name )
        {
            static tracking_state* state = get_state( );
...
#endif  // defined(ASIO_WINDOWS)
                timestamp.seconds, timestamp.microseconds, current_id, object_type, object, op_name );
        }
