    ...
    namespace detail
    {
        ...
        void kqueue_reactor::fork_service( asio::io_service::fork_event fork_ev )
        {
            if ( fork_ev == asio::io_service::fork_child )
            {
                ...
                for ( descriptor_state* state = registered_descriptors_.first( ); state != 0; state = state->next_ )
                {
                    if ( state->num_kevents_ > 0 )
                    {
                        ASIO_KQUEUE_EV_SET( &events[0], state->descriptor_, EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0,
                                            state );
                                       ...
                                       bool allow_speculative )
        {
            ...
            if ( descriptor_data->op_queue_[op_type].empty( ) )
            {
                ...
                if ( allow_speculative && ( op_type != read_op || descriptor_data->op_queue_[except_op].empty( ) ) )
                {
                    ...
                    {
                        struct kevent events[2];
                        ASIO_KQUEUE_EV_SET( &events[0], descriptor, EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0,
                                            descriptor_data );
        ...
        void kqueue_reactor::cancel_ops( socket_type, kqueue_reactor::per_descriptor_data& descriptor_data )
        {
            if ( !descriptor_data )
                return;
                                                    ...
                                                    bool closing )
        {
            ...
            if ( !descriptor_data->shutdown_ )
            {
                ...
                else
                {
                    ...
                    ASIO_KQUEUE_EV_SET( &events[1], descriptor, EVFILT_WRITE, EV_DELETE, 0, 0, 0 );
                    ::kevent( kqueue_fd_, events, descriptor_data->num_kevents_, 0, 0, 0 );
                }
        ...
        void kqueue_reactor::run( bool block, op_queue<operation>& ops )
        {
            ...
            for ( int i = 0; i < num_events; ++i )
            {
                ...
                else
                {
                    ...
                    if ( events[i].filter == EVFILT_WRITE && descriptor_data->num_kevents_ == 2 &&
                         descriptor_data->op_queue_[write_op].empty( ) )
                    {
                        ...
                        ASIO_KQUEUE_EV_SET( &delete_events[0], descriptor_data->descriptor_, EVFILT_WRITE, EV_DELETE, 0,
                                            0, 0 );
