...
namespace asio
{
    namespace detail
    {
        ...
        win_iocp_io_service::win_iocp_io_service( asio::io_service& io_service, size_t concurrency_hint )
              ...
              dispatch_required_( 0 )
        {
            ...
            iocp_.handle = ::CreateIoCompletionPort(
                ...
                static_cast<DWORD>( concurrency_hint < DWORD( ~0 ) ? concurrency_hint : DWORD( ~0 ) ) );
            if ( !iocp_.handle )
            {
        ...
        void win_iocp_io_service::shutdown_service( )
        {
            ...
            while ( ::InterlockedExchangeAdd( &outstanding_work_, 0 ) > 0 )
            {
                ...
                else
                {
                    ...
                    ::GetQueuedCompletionStatus( iocp_.handle, &bytes_transferred, &completion_key, &overlapped,
                                                 gqcs_timeout_ );
                    if ( overlapped )
                    {
                        ::InterlockedDecrement( &outstanding_work_ );
        ...
        void win_iocp_io_service::post_deferred_completions( op_queue<win_iocp_operation>& ops )
        {
            while ( win_iocp_operation* op = ops.front( ) )
            {
                ops.pop( );
        ...
        void win_iocp_io_service::abandon_operations( op_queue<win_iocp_operation>& ops )
        {
            while ( win_iocp_operation* op = ops.front( ) )
            {
                ops.pop( );
        ...
        void win_iocp_io_service::on_completion( win_iocp_operation* op, DWORD last_error, DWORD bytes_transferred )
        {
            // Flag that the operation is ready for invocation.
            op->ready_ = 1;

            // Store results in the OVERLAPPED structure.
            op->Internal = reinterpret_cast<ulong_ptr_t>( &asio::error::get_system_category( ) );
        ...
        size_t win_iocp_io_service::do_one( bool block, asio::error_code& ec )
        {
            for ( ;; )
            {
                ...
                LPOVERLAPPED overlapped = 0;
                ::SetLastError( 0 );
                BOOL ok = ::GetQueuedCompletionStatus( iocp_.handle, &bytes_transferred, &completion_key, &overlapped,
                                                       block ? gqcs_timeout_ : 0 );
                DWORD last_error = ::GetLastError( );
        ...
        void win_iocp_io_service::do_add_timer_queue( timer_queue_base& queue )
        {
            ...
            if ( !waitable_timer_.handle )
            {
                ...
                timeout.QuadPart *= 10;
                ::SetWaitableTimer( waitable_timer_.handle, &timeout, max_timeout_msec, 0, 0, FALSE );
            }
