...
namespace asio
{
    namespace detail
    {
        ...
        winrt_ssocket_service_base::winrt_ssocket_service_base( asio::io_service& io_service )
        ...
        void winrt_ssocket_service_base::construct( winrt_ssocket_service_base::base_implementation_type& impl )
        {
        ...
        void winrt_ssocket_service_base::destroy( winrt_ssocket_service_base::base_implementation_type& impl )
        {
            asio::error_code ignored_ec;
            close( impl, ignored_ec );
        ...
        asio::error_code winrt_ssocket_service_base::close( winrt_ssocket_service_base::base_implementation_type& impl,
                                                            asio::error_code& ec )
        {
            if ( impl.socket_ )
            {
        ...
        std::size_t winrt_ssocket_service_base::do_get_endpoint( const base_implementation_type& impl,
                                                                 bool local,
                                                                 void* addr,
                                                                 std::size_t addr_len,
                                                                 asio::error_code& ec ) const
        {
            ...
            try
            {
                std::string addr_string =
                    winrt_utils::string( local ? impl.socket_->Information->LocalAddress->CanonicalName
                                               : impl.socket_->Information->RemoteAddress->CanonicalName );
                unsigned short port = winrt_utils::integer( local ? impl.socket_->Information->LocalPort
                    ...
                    case ASIO_OS_DEF( AF_INET ):
                        ...
                        else
                        {
                            socket_ops::inet_pton( ASIO_OS_DEF( AF_INET ), addr_string.c_str( ),
                                                   &reinterpret_cast<sockaddr_in4_type*>( addr )->sin_addr, &scope,
                                                   ec );
            ...
            asio::error_code& ec )
        {
            ...
            try
            {
                if ( level == ASIO_OS_DEF( SOL_SOCKET ) && optname == ASIO_OS_DEF( SO_KEEPALIVE ) )
                {
                ...
                else if ( level == ASIO_OS_DEF( IPPROTO_TCP ) && optname == ASIO_OS_DEF( TCP_NODELAY ) )
                {
                    if ( optlen == sizeof( int ) )
                    {
                        int value = 0;
            ...
            asio::error_code& ec )
        {
            ...
            switch ( reinterpret_cast<const socket_addr_type*>( addr )->sa_family )
            {
                case ASIO_OS_DEF( AF_INET ):
                    socket_ops::inet_ntop( ASIO_OS_DEF( AF_INET ),
                                           &reinterpret_cast<const sockaddr_in4_type*>( addr )->sin_addr, addr_string,
                                           sizeof( addr_string ), 0, ec );
                ...
                case ASIO_OS_DEF( AF_INET6 ):
                    socket_ops::inet_ntop( ASIO_OS_DEF( AF_INET6 ),
                                           &reinterpret_cast<const sockaddr_in6_type*>( addr )->sin6_addr, addr_string,
                                           sizeof( addr_string ), 0, ec );
                    port = socket_ops::network_to_host_short(
            ...
            if ( !ec )
                try
                {
                    async_manager_.sync( impl.socket_->ConnectAsync( ref new Windows::Networking::HostName(
        ...
        void winrt_ssocket_service_base::start_connect_op( winrt_ssocket_service_base::base_implementation_type& impl,
                                                           const void* addr,
                                                           ...
                                                           bool is_continuation )
        {
            ...
            try
            {
                async_manager_.async( impl.socket_->ConnectAsync(
                                          ref new Windows::Networking::HostName( winrt_utils::string( addr_string ) ),
                                          winrt_utils::string( port ) ),
                                      op );
            }
            catch ( Platform::Exception ^ e )
        ...
        std::size_t winrt_ssocket_service_base::do_send( winrt_ssocket_service_base::base_implementation_type& impl,
                                                         ...
                                                         asio::error_code& ec )
        {
            ...
            try
            {
                buffer_sequence_adapter<asio::const_buffer, asio::const_buffers_1> bufs( asio::buffer( data ) );
                ...
                return async_manager_.sync( impl.socket_->OutputStream->WriteAsync( bufs.buffers( )[0] ), ec );
            }
            catch ( Platform::Exception ^ e )
            {
                ec = asio::error_code( e->HResult, asio::system_category( ) );
        ...
        void winrt_ssocket_service_base::start_send_op( winrt_ssocket_service_base::base_implementation_type& impl,
                                                        const asio::const_buffer& data,
                                                        socket_base::message_flags flags,
                                                        ...
                                                        bool is_continuation )
        {
            ...
            try
            {
                ...
                async_manager_.async( impl.socket_->OutputStream->WriteAsync( bufs.buffers( )[0] ), op );
            }
            catch ( Platform::Exception ^ e )
            {
                op->ec_ = asio::error_code( e->HResult, asio::system_category( ) );
        ...
        std::size_t winrt_ssocket_service_base::do_receive( winrt_ssocket_service_base::base_implementation_type& impl,
                                                            ...
                                                            asio::error_code& ec )
        {
            ...
            try
            {
                buffer_sequence_adapter<asio::mutable_buffer, asio::mutable_buffers_1> bufs( asio::buffer( data ) );
                ...
                async_manager_.sync(
                    impl.socket_->InputStream->ReadAsync( bufs.buffers( )[0], bufs.buffers( )[0]->Capacity,
        ...
        void winrt_ssocket_service_base::start_receive_op( winrt_ssocket_service_base::base_implementation_type& impl,
                                                           const asio::mutable_buffer& data,
                                                           socket_base::message_flags flags,
