...
namespace asio
{
    namespace detail
    {
        ...
        template <typename Handler, typename T>
        inline void asio_handler_deallocate( void* pointer, std::size_t size, coro_handler<Handler, T>* this_handler )
        {
    ...
    struct handler_type<basic_yield_context<Handler>, ReturnType( asio::error_code, Arg2 )>
    {
    ...
    class async_result<detail::coro_handler<Handler, T>>
    {
       public:
        ...
        explicit async_result( detail::coro_handler<Handler, T>& h ) : handler_( h ), ca_( h.ca_ ), ready_( 2 )
        {
            h.ready_ = &ready_;
            out_ec_ = h.ec_;
            if ( !out_ec_ )
    ...
    class async_result<detail::coro_handler<Handler, void>>
    {
       public:
        ...
        explicit async_result( detail::coro_handler<Handler, void>& h ) : handler_( h ), ca_( h.ca_ ), ready_( 2 )
        {
            h.ready_ = &ready_;
            out_ec_ = h.ec_;
            if ( !out_ec_ )
                ...
                const boost::coroutines::attributes& attributes )
    {
        ...
        helper.data_.reset( new detail::spawn_data<Handler, Function>( ASIO_MOVE_CAST( Handler )( handler ), true,
                ...
                const boost::coroutines::attributes& attributes )
    {
        ...
        detail::spawn_helper<Handler, Function> helper;
        helper.data_.reset( new detail::spawn_data<Handler, Function>( ASIO_MOVE_CAST( Handler )( handler ), false,
                                                                       ASIO_MOVE_CAST( Function )( function ) ) );
        helper.attributes_ = attributes;
        asio_handler_invoke_helpers::invoke( helper, helper.data_->handler_ );
    }
    ...
    void spawn( asio::io_service::strand strand,
                ...
                const boost::coroutines::attributes& attributes )
    {
        asio::spawn( strand.wrap( &detail::default_spawn_handler ), ASIO_MOVE_CAST( Function )( function ),
                     attributes );
    }
    ...
    void spawn( asio::io_service& io_service,
                ...
                const boost::coroutines::attributes& attributes )
    {
        asio::spawn( asio::io_service::strand( io_service ), ASIO_MOVE_CAST( Function )( function ), attributes );
    }
