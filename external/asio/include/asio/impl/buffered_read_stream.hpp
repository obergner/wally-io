...
namespace asio
{
    ...
    std::size_t buffered_read_stream<Stream>::fill( asio::error_code& ec )
    {
        ...
        storage_.resize(
            previous_size +
            next_layer_.read_some( buffer( storage_.data( ) + previous_size, storage_.size( ) - previous_size ), ec ) );
        resize_guard.commit( );
        return storage_.size( ) - previous_size;
    }
    ...
    namespace detail
    {
        ...
        template <typename ReadHandler>
        inline void* asio_handler_allocate( std::size_t size, buffered_fill_handler<ReadHandler>* this_handler )
        {
        ...
        inline void asio_handler_invoke( Function& function, buffered_fill_handler<ReadHandler>* this_handler )
        {
            asio_handler_invoke_helpers::invoke( function, this_handler->handler_ );
        }

        template <typename Function, typename ReadHandler>
        inline void asio_handler_invoke( const Function& function, buffered_fill_handler<ReadHandler>* this_handler )
        {
            asio_handler_invoke_helpers::invoke( function, this_handler->handler_ );
        }
    }  // namespace detail
    ...
    buffered_read_stream<Stream>::async_fill( ASIO_MOVE_ARG( ReadHandler ) handler )
    {
        ...
        next_layer_.async_read_some(
            buffer( storage_.data( ) + previous_size, storage_.size( ) - previous_size ),
            detail::buffered_fill_handler<ASIO_HANDLER_TYPE( ReadHandler, void( asio::error_code, std::size_t ) )>(
        ...
        class buffered_read_some_handler
        {
           public:
            buffered_read_some_handler( detail::buffered_stream_storage& storage,
                                        ...
                                        ReadHandler& handler )
                : storage_( storage ), buffers_( buffers ), handler_( handler )
            {
            }
        ...
        if ( asio::buffer_size( buffers ) == 0 || !storage_.empty( ) )
        {
            next_layer_.async_read_some(
                ...
                detail::buffered_read_some_handler<
                    MutableBufferSequence, ASIO_HANDLER_TYPE( ReadHandler, void( asio::error_code, std::size_t ) )>(
