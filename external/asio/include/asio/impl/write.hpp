...
namespace asio
{
    ...
    namespace detail
    {
        ...
        template <typename AsyncWriteStream, typename CompletionCondition, typename WriteHandler>
        class write_op<AsyncWriteStream, asio::mutable_buffers_1, CompletionCondition, WriteHandler>
            ...
            void operator( )( const asio::error_code& ec, std::size_t bytes_transferred, int start = 0 )
            {
                ...
                switch ( start_ = start )
                {
                    case 1:
                        ...
                        for ( ;; )
                        {
                            stream_.async_write_some( asio::buffer( buffer_ + total_transferred_, n ),
        ...
        template <typename AsyncWriteStream, typename CompletionCondition, typename WriteHandler>
        class write_op<AsyncWriteStream, asio::const_buffers_1, CompletionCondition, WriteHandler>
            : detail::base_from_completion_cond<CompletionCondition>
        {
           public:
            write_op( AsyncWriteStream& stream,
        ...
        class write_op<AsyncWriteStream, boost::array<Elem, 2>, CompletionCondition, WriteHandler>
            : detail::base_from_completion_cond<CompletionCondition>
        {
           public:
            write_op( AsyncWriteStream& stream,
        ...
        template <typename AsyncWriteStream, typename Elem, typename CompletionCondition, typename WriteHandler>
        class write_op<AsyncWriteStream, std::array<Elem, 2>, CompletionCondition, WriteHandler>
            : detail::base_from_completion_cond<CompletionCondition>
        {
           public:
            write_op( AsyncWriteStream& stream,
        ...
        inline void* asio_handler_allocate(
            std::size_t size,
            write_op<AsyncWriteStream, ConstBufferSequence, CompletionCondition, WriteHandler>* this_handler )
        {
                     ...
                     ASIO_MOVE_ARG( WriteHandler ) handler )
    {
        ...
        detail::write_op<AsyncWriteStream, ConstBufferSequence, CompletionCondition,
                         ASIO_HANDLER_TYPE( WriteHandler, void( asio::error_code, std::size_t ) )>(
            s, buffers, completion_condition, init.handler )( asio::error_code( ), 0, 1 );
    ...
    inline ASIO_INITFN_RESULT_TYPE( WriteHandler, void( asio::error_code, std::size_t ) )
        async_write( AsyncWriteStream& s, const ConstBufferSequence& buffers, ASIO_MOVE_ARG( WriteHandler ) handler )
    {
        // If you get an error on the following line it means that your handler does
        // not meet the documented type requirements for a WriteHandler.
        ASIO_WRITE_HANDLER_CHECK( WriteHandler, handler ) type_check;
        ...
        detail::write_op<AsyncWriteStream, ConstBufferSequence, detail::transfer_all_t,
                         ASIO_HANDLER_TYPE( WriteHandler, void( asio::error_code, std::size_t ) )>(
            s, buffers, transfer_all( ), init.handler )( asio::error_code( ), 0, 1 );
    ...
    namespace detail
    {
        ...
        template <typename Allocator, typename WriteHandler>
        inline bool asio_handler_is_continuation( write_streambuf_handler<Allocator, WriteHandler>* this_handler )
        {
                     ...
                     ASIO_MOVE_ARG( WriteHandler ) handler )
    {
        ...
        async_write( s, b.data( ), completion_condition,
                     detail::write_streambuf_handler<Allocator, ASIO_HANDLER_TYPE( WriteHandler, void( asio::error_code,
                                                                                                       std::size_t ) )>(
                         b, init.handler ) );
    ...
    inline ASIO_INITFN_RESULT_TYPE( WriteHandler, void( asio::error_code, std::size_t ) )
        async_write( AsyncWriteStream& s, asio::basic_streambuf<Allocator>& b, ASIO_MOVE_ARG( WriteHandler ) handler )
    {
        // If you get an error on the following line it means that your handler does
        // not meet the documented type requirements for a WriteHandler.
        ASIO_WRITE_HANDLER_CHECK( WriteHandler, handler ) type_check;
