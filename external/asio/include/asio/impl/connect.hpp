...
namespace asio
{
    ...
    template <typename Protocol, typename SocketService, typename Iterator>
    inline Iterator connect( basic_socket<Protocol, SocketService>& s, Iterator begin, asio::error_code& ec )
    {
    ...
    namespace detail
    {
                ...
                switch ( start_ = start )
                {
                    case 1:
                        for ( ;; )
                        {
                            ...
                            {
                                socket_.close( ec );
                                socket_.async_connect( *iter_, ASIO_MOVE_CAST( connect_op )( *this ) );
                                return;
                            }
                        ...
                        handler_( static_cast<const asio::error_code&>( ec ), static_cast<const Iterator&>( iter_ ) );
                }
            }
        ...
        inline void* asio_handler_allocate(
            std::size_t size,
            connect_op<Protocol, SocketService, Iterator, ConnectCondition, ComposedConnectHandler>* this_handler )
        {
    ...
    inline ASIO_INITFN_RESULT_TYPE( ComposedConnectHandler, void( asio::error_code, Iterator ) )
                       ...
                       ASIO_MOVE_ARG( ComposedConnectHandler ) handler )
    {
        ...
        ASIO_COMPOSED_CONNECT_HANDLER_CHECK( ComposedConnectHandler, handler, Iterator ) type_check;

        detail::async_result_init<ComposedConnectHandler, void( asio::error_code, Iterator )> init(
            ASIO_MOVE_CAST( ComposedConnectHandler )( handler ) );

        detail::connect_op<Protocol, SocketService, Iterator, detail::default_connect_condition,
                           ASIO_HANDLER_TYPE( ComposedConnectHandler, void( asio::error_code, Iterator ) )>(
            s, begin, Iterator( ), detail::default_connect_condition( ), init.handler )( asio::error_code( ), 1 );
                       ...
                       ASIO_MOVE_ARG( ComposedConnectHandler ) handler )
    {
        ...
        detail::connect_op<Protocol, SocketService, Iterator, detail::default_connect_condition,
                           ASIO_HANDLER_TYPE( ComposedConnectHandler, void( asio::error_code, Iterator ) )>(
            s, begin, end, detail::default_connect_condition( ), init.handler )( asio::error_code( ), 1 );
                       ...
                       ASIO_MOVE_ARG( ComposedConnectHandler ) handler )
    {
        ...
        detail::connect_op<Protocol, SocketService, Iterator, ConnectCondition,
                           ASIO_HANDLER_TYPE( ComposedConnectHandler, void( asio::error_code, Iterator ) )>(
            s, begin, Iterator( ), connect_condition, init.handler )( asio::error_code( ), 1 );
                       ...
                       ASIO_MOVE_ARG( ComposedConnectHandler ) handler )
    {
        ...
        detail::connect_op<Protocol, SocketService, Iterator, ConnectCondition,
                           ASIO_HANDLER_TYPE( ComposedConnectHandler, void( asio::error_code, Iterator ) )>(
            s, begin, end, connect_condition, init.handler )( asio::error_code( ), 1 );
