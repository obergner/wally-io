        ...
        class stream : public stream_base, private noncopyable
        {
           public:
            ...
            stream( Arg& arg, context& ctx )
                : next_layer_( arg ), core_( ctx.native_handle( ), next_layer_.lowest_layer( ).get_io_service( ) )
            {
            ...
            {
                return core_.engine_.set_verify_callback( new detail::verify_callback<VerifyCallback>( callback ), ec );
            }
            ...
            {
                detail::io( next_layer_, core_, detail::buffered_handshake_op<ConstBufferSequence>( type, buffers ),
                            ec );
                return ec;
            }
            ...
            async_handshake( handshake_type type, ASIO_MOVE_ARG( HandshakeHandler ) handler )
            {
                ...
                asio::detail::async_result_init<HandshakeHandler, void( asio::error_code )> init(
                    ASIO_MOVE_CAST( HandshakeHandler )( handler ) );

                detail::async_io( next_layer_, core_, detail::handshake_op( type ), init.handler );
                             ...
                             ASIO_MOVE_ARG( BufferedHandshakeHandler ) handler )
            {
                ...
                asio::detail::async_result_init<BufferedHandshakeHandler, void( asio::error_code, std::size_t )> init(
                    ASIO_MOVE_CAST( BufferedHandshakeHandler )( handler ) );

                detail::async_io( next_layer_, core_,
                                  detail::buffered_handshake_op<ConstBufferSequence>( type, buffers ), init.handler );
            ...
            async_shutdown( ASIO_MOVE_ARG( ShutdownHandler ) handler )
            {
                ...
                asio::detail::async_result_init<ShutdownHandler, void( asio::error_code )> init(
            ...
            async_write_some( const ConstBufferSequence& buffers, ASIO_MOVE_ARG( WriteHandler ) handler )
            {
                ...
                asio::detail::async_result_init<WriteHandler, void( asio::error_code, std::size_t )> init(
                    ASIO_MOVE_CAST( WriteHandler )( handler ) );

                detail::async_io( next_layer_, core_, detail::write_op<ConstBufferSequence>( buffers ), init.handler );
            ...
            async_read_some( const MutableBufferSequence& buffers, ASIO_MOVE_ARG( ReadHandler ) handler )
            {
                ...
                asio::detail::async_result_init<ReadHandler, void( asio::error_code, std::size_t )> init(
                    ASIO_MOVE_CAST( ReadHandler )( handler ) );

                detail::async_io( next_layer_, core_, detail::read_op<MutableBufferSequence>( buffers ), init.handler );
