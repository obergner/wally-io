...
namespace asio
{
    namespace ssl
    {
        ...
        context::context( asio::io_service&, context::method m ) : handle_( 0 )
        {
            context tmp( m );
            handle_ = tmp.handle_;
            tmp.handle_ = 0;
            ...
            if ( handle_ )
            {
                ...
                if ( SSL_CTX_get_app_data( handle_ ) )
                {
                    detail::verify_callback_base* callback =
                        static_cast<detail::verify_callback_base*>( SSL_CTX_get_app_data( handle_ ) );
                    delete callback;
        ...
        asio::error_code context::clear_options( context::options o, asio::error_code& ec )
        {
        ...
        asio::error_code context::set_options( context::options o, asio::error_code& ec )
        {
#if !defined( SSL_OP_NO_COMPRESSION )
            if ( ( o & context::no_compression ) != 0 )
            {
#if ( OPENSSL_VERSION_NUMBER >= 0x00908000L )
                handle_->comp_methods = asio::ssl::detail::openssl_init<>::get_null_compression_methods( );
#endif  // (OPENSSL_VERSION_NUMBER >= 0x00908000L)
                o ^= context::no_compression;
            }
#endif  // !defined(SSL_OP_NO_COMPRESSION)
        ...
        asio::error_code context::set_verify_mode( verify_mode v, asio::error_code& ec )
        {
        ...
        asio::error_code context::set_verify_depth( int depth, asio::error_code& ec )
        {
        ...
        asio::error_code context::load_verify_file( const std::string& filename, asio::error_code& ec )
        {
            ::ERR_clear_error( );
        ...
        asio::error_code context::add_certificate_authority( const const_buffer& ca, asio::error_code& ec )
        {
            ::ERR_clear_error( );
        ...
        asio::error_code context::set_default_verify_paths( asio::error_code& ec )
        {
            ::ERR_clear_error( );
        ...
        asio::error_code context::add_verify_path( const std::string& path, asio::error_code& ec )
        {
            ::ERR_clear_error( );
        ...
        asio::error_code context::use_certificate_file( const std::string& filename,
                                                        file_format format,
                                                        asio::error_code& ec )
        {
            int file_type;
        ...
        asio::error_code context::use_certificate_chain( const const_buffer& chain, asio::error_code& ec )
        {
            ::ERR_clear_error( );
            ...
            if ( bio.p )
            {
                ...
                void* cb_userdata = handle_->default_passwd_callback_userdata;
#endif  // (OPENSSL_VERSION_NUMBER >= 0x10100000L)
                x509_cleanup cert = {::PEM_read_bio_X509_AUX( bio.p, 0, callback, cb_userdata )};
                if ( !cert.p )
                {
                    ec = asio::error_code( ERR_R_PEM_LIB, asio::error::get_ssl_category( ) );
                    return ec;
                }
                ...
                while ( X509* cacert = ::PEM_read_bio_X509( bio.p, 0, callback, cb_userdata ) )
                {
                ...
                if ( ( ERR_GET_LIB( result ) == ERR_LIB_PEM ) && ( ERR_GET_REASON( result ) == PEM_R_NO_START_LINE ) )
                {
                    ::ERR_clear_error( );
                    ec = asio::error_code( );
                    return ec;
        ...
        asio::error_code context::use_certificate_chain_file( const std::string& filename, asio::error_code& ec )
        {
            ::ERR_clear_error( );
        ...
        void context::use_private_key( const const_buffer& private_key, context::file_format format )
        {
            asio::error_code ec;
        ...
        asio::error_code context::use_private_key( const const_buffer& private_key,
            ...
            if ( bio.p )
            {
                ...
                switch ( format )
                {
                    case context_base::asn1:
                        ...
                        break;
                    case context_base::pem:
                        evp_private_key.p = ::PEM_read_bio_PrivateKey( bio.p, 0, callback, cb_userdata );
                        break;
                    default:
                    {
                        ec = asio::error::invalid_argument;
        ...
        void context::use_private_key_file( const std::string& filename, context::file_format format )
        {
            asio::error_code ec;
        ...
        void context::use_rsa_private_key( const const_buffer& private_key, context::file_format format )
        {
            asio::error_code ec;
        ...
        asio::error_code context::use_rsa_private_key( const const_buffer& private_key,
                                                       context::file_format format,
                                                       asio::error_code& ec )
        {
            ::ERR_clear_error( );
            ...
            if ( bio.p )
            {
                ...
                switch ( format )
                {
                    case context_base::asn1:
                        ...
                        break;
                    case context_base::pem:
                        rsa_private_key.p = ::PEM_read_bio_RSAPrivateKey( bio.p, 0, callback, cb_userdata );
                        break;
                    default:
                    {
                        ec = asio::error::invalid_argument;
        ...
        asio::error_code context::use_private_key_file( const std::string& filename,
                                                        context::file_format format,
                                                        asio::error_code& ec )
        {
            int file_type;
        ...
        void context::use_rsa_private_key_file( const std::string& filename, context::file_format format )
        {
            asio::error_code ec;
        ...
        asio::error_code context::use_rsa_private_key_file( const std::string& filename,
                                                            context::file_format format,
                                                            asio::error_code& ec )
        {
            int file_type;
        ...
        asio::error_code context::use_tmp_dh( const const_buffer& dh, asio::error_code& ec )
        {
            ::ERR_clear_error( );
        ...
        asio::error_code context::use_tmp_dh_file( const std::string& filename, asio::error_code& ec )
        {
            ::ERR_clear_error( );
        ...
        asio::error_code context::do_use_tmp_dh( BIO* bio, asio::error_code& ec )
        {
            ::ERR_clear_error( );
        ...
        asio::error_code context::do_set_verify_callback( detail::verify_callback_base* callback, asio::error_code& ec )
        {
            if ( SSL_CTX_get_app_data( handle_ ) )
            {
                delete static_cast<detail::verify_callback_base*>( SSL_CTX_get_app_data( handle_ ) );
            }

            SSL_CTX_set_app_data( handle_, callback );

            ::SSL_CTX_set_verify( handle_, ::SSL_CTX_get_verify_mode( handle_ ), &context::verify_callback_function );
            ...
            if ( ctx )
            {
                         ...
                         ::X509_STORE_CTX_get_ex_data( ctx, ::SSL_get_ex_data_X509_STORE_CTX_idx( ) ) ) )
                {
                    if ( SSL_CTX* handle = ::SSL_get_SSL_CTX( ssl ) )
                    {
                        ...
                        {
                            detail::verify_callback_base* callback =
                                static_cast<detail::verify_callback_base*>( SSL_CTX_get_app_data( handle ) );
        ...
        asio::error_code context::do_set_password_callback( detail::password_callback_base* callback,
                                                            asio::error_code& ec )
        {
#if ( OPENSSL_VERSION_NUMBER >= 0x10100000L )
        ...
        int context::password_callback_function( char* buf, int size, int purpose, void* data )
        {
            using namespace std;  // For strncat and strlen.

            if ( data )
            {
                detail::password_callback_base* callback = static_cast<detail::password_callback_base*>( data );
        ...
        {
            return ::BIO_new_mem_buf( const_cast<void*>( buffer_cast<const void*>( b ) ),
