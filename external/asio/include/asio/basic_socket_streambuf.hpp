                                   ...
                                   public basic_socket<Protocol, StreamSocketService>
    {
       ...
       public:
        ...
        basic_socket_streambuf( )
            : basic_socket<Protocol, StreamSocketService>( this->detail::socket_streambuf_base::io_service_ ),
              unbuffered_( false ),
              timer_service_( 0 ),
         ...
         */
        template <typename T1, ..., typename TN>
        basic_socket_streambuf<Protocol, StreamSocketService>* connect( T1 t1, ..., TN tn );
#elif defined( ASIO_HAS_VARIADIC_TEMPLATES )
        template <typename... T>
        basic_socket_streambuf<Protocol, StreamSocketService, Time, TimeTraits, TimerService>* connect( T... x )
        {
            init_buffers( );
            this->basic_socket<Protocol, StreamSocketService>::close( ec_ );
            typedef typename Protocol::resolver resolver_type;
        ...
        {
            return timer_service_ ? timer_service_->expires_at( timer_implementation_ ) : time_type( );
        }
        ...
        int_type overflow( int_type c )
        {
            ...
            else
            {
                ...
                while ( asio::buffer_size( buffer ) > 0 )
                {
                    ...
                    io_handler handler = {this};
                    this->get_service( ).async_send( this->get_implementation( ), asio::buffer( buffer ), 0, handler );
       ...
       private:
        ...
        void resolve_and_connect( const ResolverQuery& query )
        {
            ...
            if ( !ec_ )
            {
                ...
                while ( ec_ && i != end )
                {
                    ...
                    io_handler handler = {this};
                    this->basic_socket<Protocol, StreamSocketService>::async_connect( *i, handler );
        ...
        struct io_handler
        {
            ...
            void operator( )( const asio::error_code& ec, std::size_t bytes_transferred = 0 )
            {
                this_->ec_ = ec;
        ...
        struct timer_handler
        {
            ...
            void operator( )( const asio::error_code& )
            {
                ...
                time_type expiry_time = this_->timer_service_->expires_at( this_->timer_implementation_ );
        ...
        void construct_timer( )
        {
            if ( timer_service_ == 0 )
            {
                TimerService& timer_service = use_service<TimerService>( detail::socket_streambuf_base::io_service_ );
