...
namespace asio
{
    ...
    class buffered_write_stream : private noncopyable
    {
       public:
        ...
        /// Construct, passing the specified argument to initialise the next layer.
        template <typename Arg>
        buffered_write_stream( Arg& a, std::size_t buffer_size ) : next_layer_( a ), storage_( buffer_size )
        {
        }
        ...
        async_read_some( const MutableBufferSequence& buffers, ASIO_MOVE_ARG( ReadHandler ) handler )
        {
            ...
            next_layer_.async_read_some(
                buffers, ASIO_MOVE_CAST( ASIO_HANDLER_TYPE( ReadHandler, void( asio::error_code, std::size_t ) ) )(
                             init.handler ) );
