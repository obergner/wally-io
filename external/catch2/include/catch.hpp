...
#ifdef _MSC_VER
#pragma warning( push )
#pragma warning( disable : 4180 )  // We attempt to stream a function (address) by const&, which MSVC complains about
                                   // but is harmless
#endif
...
namespace Catch
{
    ...
    // Specialised comparison functions to handle equality comparisons between ints and pointers (NULL deduces as an
    // int)
    template <typename LhsT, typename RhsT>
...
#define INTERNAL_CATCH_TEST( macroName, resultDisposition, ... )                                                     \
    ...
    } while ( (void)0, ( false ) && static_cast<bool>(                                                               \
                                        !!( __VA_ARGS__ ) ) )  // the expression here is never evaluated at runtime but
...
namespace Catch
{
    namespace Generators
    {
        ...
        {
            return GeneratorWrapper<T>( std::unique_ptr<IGenerator<T>>( pf::make_unique<FilterGenerator<T, Predicate>>(
...
namespace Catch
{
    namespace Detail
    {
        ...
        {
            CATCH_ENFORCE( newMargin >= 0, "Invalid Approx::margin: " << newMargin << '.'
...
#define CATCH_CHECK_THROWS_WITH( expr, matcher )                                                               \
    INTERNAL_CATCH_THROWS_STR_MATCHES( "CATCH_CHECK_THROWS_WITH", Catch::ResultDisposition::ContinueOnFailure, \
                                       matcher, expr )
#if !defined( CATCH_CONFIG_DISABLE_MATCHERS )
